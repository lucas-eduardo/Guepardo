<?php
	...
	class GU_PDO
	{
		...
		public function conectar( $tipobanco=null, $host=null, $banco=null, $usuariobanco=null, $senhabanco=null )
		{
			...
			try
			{
				...
				if( !is_null($tipobanco) && !is_null($host) && !is_null($banco) && !is_null($usuariobanco) && !is_null($senhabanco) ){
					...
					// Monta a conexão com o banco de dados, fora do arquivo bancodedados.php
					...
					// Monta a conexão com o banco de dados com base no arquivo bancodedados.php
				...
				// Se a conexão foi feita com sucesso
		...
		public function setselect( $select=null )
		{
			...
			if( !is_null( $select ) ){ // Se o select for diferente de null
		...
		private function montawhere( $where )
		{
			...
			if( !is_null( $where ) && !empty ($where) ){ // Se existir o where...

				$auxw=0;

				foreach($where as $w){ // Roda um loop no where
